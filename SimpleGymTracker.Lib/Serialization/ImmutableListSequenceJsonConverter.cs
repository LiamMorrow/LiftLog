...
namespace SimpleGymTracker.Lib.Serialization
{
  public class ImmutableListSequenceJsonConverter : JsonConverterFactory
  {
    ...
    {
      var converterType = typeof(ImmutableListSequenceJsonConverterInner<>)
        .MakeGenericType(new[] { type.GetGenericArguments()[0] });
      return (JsonConverter?)Activator.CreateInstance(converterType, new[] { options });
    }

    private class ImmutableListSequenceJsonConverterInner<T> : JsonConverter<ImmutableListSequence<T>>
    {
      ...
      public override ImmutableListSequence<T>? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
      {
      ...
      public override void Write(Utf8JsonWriter writer, ImmutableListSequence<T> value, JsonSerializerOptions options)
      {
