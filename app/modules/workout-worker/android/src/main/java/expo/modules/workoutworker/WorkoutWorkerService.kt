...
class WorkoutWorkerService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        // Handle the finish workout action from notification
        if (intent?.action == WorkoutConstants.ACTION_FINISH_WORKOUT) {
            Log.d("WorkoutWorker", "Finish workout action received")
            val event =
                WorkoutMessageOuterClass.WorkoutMessage.newBuilder().setFinishWorkoutCommand(
                    WorkoutMessageOuterClass.FinishWorkoutCommand.getDefaultInstance()
                ).build()
            eventDispatch("FinishWorkoutCommand", event)
            return START_STICKY
        }

        val notification = notificationManager.createInitialNotificationBuilder().build()
        ...
        ServiceCompat.startForeground(
            this, WorkoutNotificationManager.NOTIFICATION_ID, notification, type
        )
