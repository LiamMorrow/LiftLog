...
class WorkoutUpdatedHandler(
    ...
    override suspend fun handle(
        ...
        when {
            workoutUpdatedEvent.hasTimerInfo() -> showTimerNotification(event)
    ...
    private fun showFinishedNotification(event: WorkoutMessageOuterClass.WorkoutMessage) {
        ...
        val message = messageTemplate.replace(
            "\$WEIGHT$",
            formatWeight(event.workoutUpdatedEvent.totalWeightLifted)
        ).replace(
            "\$TIME$",
            formatDuration(fromDurationDao(event.workoutUpdatedEvent.workoutDuration))
        )

        val notifBuilder = notificationManager.createWorkoutNotificationBuilder(event.translations)
    ...
    private fun showTimerNotification(
        ...
        val timerInfo = workoutUpdatedEvent.timerInfo
        val currentExerciseMessage = getCurrentExerciseMessage(event)
        timer.updateCallback {
            val timeStartSecs = timerInfo.startedAt.seconds
            val timePartiallyEndSecs = timerInfo.partiallyEndAt.seconds
            val timeEndSecs = timerInfo.endAt.seconds
            ...
            var notifBuilder =
                notificationManager.createWorkoutNotificationBuilder(event.translations)
