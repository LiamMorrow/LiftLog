 ...
 * from the previous version's shape into the next. The version field is managed
 * automatically and must be omitted from both the input and output of each `up`
 * function. Finish the chain with `.build()` to obtain a {@link Migrator}.
 *
 ...
 *
 * @example
 * // Define types for each stored version:
 * type V1 = { version: number; name: string };
 * type V2 = { version: number; name: string; displayName: string };
 * type V3 = { version: number; name: string; displayName: string; age: number };
 *
