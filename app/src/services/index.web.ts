...
class WebDatabaseMigrationService {
  constructor(
  ...
  async migrate(): Promise<void> {
    void this.db;
    void this.keyValueStore;
    void this.preferenceService;
  }
}

function createWebDbProxy(): ExpoSQLiteDatabase {
  return new Proxy(
    {},
    {
      get(_target, prop) {
        throw new Error(
          `SQLite is not available on web in LiftLog (attempted to access db.${String(prop)}).`,
        );
      },
    },
  ) as ExpoSQLiteDatabase;
}

const db = createWebDbProxy();
...
function resolveServicesInternal(store: Store<RootState>) {
  ...
  const databaseMigrationService = new WebDatabaseMigrationService(
    db,
    keyValueStore,
    preferenceService,
  );
