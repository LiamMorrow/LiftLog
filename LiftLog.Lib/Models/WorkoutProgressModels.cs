...
namespace LiftLog.Lib.Models
{
    ...
    public record WorkoutWeightedExercise(
        WorkoutPlanWeightedExercise PlanExercise,
        ImmutableListSequence<WorkoutSet?> Sets,
        decimal Weight
    )
    {
    ...
    )
    {
        public WorkoutWeightedExercise? NextExercise =>
            WeightedExercises.FirstOrDefault(
                x => x.Sets.Any(set => set is null) && !x.Sets.All(reps => reps is null)
            );

        public WorkoutWeightedExercise? LastExercise =>
            WeightedExercises.LastOrDefault(
                x => x.Sets.Any(set => set is not null) && !x.Sets.All(reps => reps is not null)
            );
    }

    public record WorkoutDayDao(Guid Id, WorkoutDay Day);
}
